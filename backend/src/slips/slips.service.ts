    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (receiptId) {
        if (type !== SlipType.PAYMENT) {
          throw new BadRequestException('Liên kết với hóa đơn nhập kho (Receipt) chỉ được dùng cho Phiếu chi (PAYMENT).');
        }
        ...
        const currentDebt = Number(receipt.postTaxTotal) - Number(receipt.paidAmount);
        ...
        if (type !== SlipType.RECEIPT) {
          throw new BadRequestException('Liên kết với hóa đơn xuất kho (Export) chỉ được dùng cho Phiếu thu (RECEIPT).');
        }
        ...
        const currentDebt = Number(exportBill.postTaxTotal) - Number(exportBill.paidAmount);
        if (amount > currentDebt) {
          throw new BadRequestException(
