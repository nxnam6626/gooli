  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (container) {
        ...
        const scrollStep =
          window.innerWidth >= 1024 ? clientWidth / 4 : clientWidth;
    ...
    >
      ...
      <div className="relative group/carousel">
        ...
        <button
          ...
          className={`absolute left-[-16px] top-[40%] -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white dark:bg-neutral-800 shadow-lg border border-neutral-100 dark:border-neutral-700 flex items-center justify-center text-neutral-600 dark:text-neutral-300 transition-all duration-300 cursor-pointer ${
            isHovered
        ...
        <button
          ...
          className={`absolute right-[-16px] top-[40%] -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-white dark:bg-neutral-800 shadow-lg border border-neutral-100 dark:border-neutral-700 flex items-center justify-center text-neutral-600 dark:text-neutral-300 transition-all duration-300 cursor-pointer ${
            isHovered
              ? 'opacity-100 translate-x-0'
              : 'opacity-0 translate-x-2 pointer-events-none'
          } hover:bg-neutral-50 dark:hover:bg-neutral-700 hover:scale-105 active:scale-95`}
            ...
            >
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <h3
                    className={`text-sm md:text-base font-extrabold uppercase tracking-wide text-left mb-1 line-clamp-1 ${cat.colorTheme.textColor}`}
                  >
                    {cat.title}
                  </h3>
                  <span
                    className={`text-xs font-bold transition-transform duration-300 group-hover/card:translate-x-1 ${cat.colorTheme.textColor}`}
                  >
                    →
                  </span>
                </div>
                <p
                  className={`text-xs md:text-sm text-left leading-relaxed line-clamp-1 font-medium opacity-85 ${cat.colorTheme.textColor}`}
                >
                  {cat.desc}
                </p>
              </div>
