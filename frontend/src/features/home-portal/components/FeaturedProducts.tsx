...
export default function FeaturedProducts() {
      ...
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          parsed.forEach((cat: { href: string; label: string }) => {
            if (cat.href && cat.href.endsWith('la-phong') && cat.label)
              newTabs[0].label = cat.label;
            if (cat.href && cat.href.endsWith('lam-trong-nha') && cat.label)
              newTabs[1].label = cat.label;
            if (cat.href && cat.href.endsWith('lam-ngoai-troi') && cat.label)
              newTabs[2].label = cat.label;
            if (cat.href && cat.href.endsWith('tam-nano') && cat.label)
              newTabs[3].label = cat.label;
          });
        ...
        console.error(
          'Failed to parse website categories in featured products:',
          err,
        );
      }
    }
  ...
  const activeHref =
    activeTab === 'la-phong'
      ? '/san-pham/la-phong'
      : activeTab === 'lam-trong-nha'
        ? '/san-pham/lam-trong-nha'
        : activeTab === 'lam-ngoai-troi'
          ? '/san-pham/lam-ngoai-troi'
          : '/san-pham/tam-nano';
    ...
    <section className="w-full">
      ...
      >
        ...
        <div className="flex flex-nowrap overflow-x-auto scrollbar-none items-center gap-1.5 sm:gap-2 md:gap-3 w-full sm:w-auto pb-2 sm:pb-0">
          ...
          <Link
            href={activeHref}
            className="view-all-btn-pill cursor-pointer shrink-0"
          >
            Xem tất cả
          </Link>
        </div>
        ...
        {activeProducts.map((prod) => (
          ...
          >
            ...
            >
              ...
              <div className="product-card-btn select-none">Xem chi tiết</div>
            </div>
          </Link>
        ))}
      </div>
