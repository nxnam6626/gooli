      ...
      <div className="flex justify-between items-center select-none">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-[11px]">
            Cập nhật và theo dõi tồn kho theo thời gian thực.
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm border-collapse">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500">
              <tr>
                <th className="px-4 py-3.5 font-bold text-[11px] uppercase tracking-wider w-8 text-center">
                  #
                </th>
                <th className="px-4 py-3.5 font-bold text-[11px] uppercase tracking-wider">
                  Số phiếu
                </th>
                <th className="px-4 py-3.5 font-bold text-[11px] uppercase tracking-wider">
                  Khách hàng
                </th>
                <th className="px-4 py-3.5 font-bold text-[11px] uppercase tracking-wider text-right">
                  SL
                </th>
                <th className="px-4 py-3.5 font-bold text-[11px] uppercase tracking-wider text-right">
                  Tổng trước thuế
                ...
                {perms.approve_bills && (
                  <th className="px-4 py-3.5 font-bold text-[11px] uppercase tracking-wider text-center">
                    Thao tác
                  </th>
                )}
              </tr>
                    ...
                    <React.Fragment key={ex.id}>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-mono font-bold text-slate-900 text-xs flex items-center gap-2.5 h-[45px]">
                          {isExpanded ? (
                            <CaretUp size={12} className="text-[#2563eb]" />
                          ) : (
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded-full border ${st.color}`}
                          >
                            {st.label}
                          </span>
                        </td>
                      ...
                      {isExpanded && ex.items.length > 0 && (
                        <tr>
                          <td
                            colSpan={perms.approve_bills ? 9 : 8}
                            className="px-0 py-0 bg-slate-50/50"
                          >
                              ...
                              <table className="w-full text-xs border-collapse">
                                <thead>
                                  <tr className="text-slate-400 border-b border-slate-200/70">
                                    <th className="text-left pb-2 font-bold uppercase text-[10px]">
                                      Tên hàng
                                    </th>
                                    <th className="text-right pb-2 font-bold uppercase text-[10px] w-24">
                                      Số lượng
                                    </th>
                                    <th className="text-right pb-2 font-bold uppercase text-[10px] w-32">
                                      Đơn giá
                                    </th>
                                    <th className="text-right pb-2 font-bold uppercase text-[10px] w-20">
                                      VAT
                                    </th>
                                    <th className="text-right pb-2 font-bold uppercase text-[10px] w-32">
                                      Thành tiền
        ...
        {!loading && exports.length > 0 && (
          <div className="px-4 py-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-3 text-sm">
            <span className="text-slate-500 text-xs">
              Tổng cộng{' '}
              <strong className="text-slate-800">{exports.length}</strong> phiếu
              xuất · Bấm vào dòng phiếu để xem chi tiết
            </span>
            <span className="font-mono text-slate-900 font-extrabold text-base">
              ...
              <span className="text-[#2563eb]">
                {fmt(
                  exports.reduce(
                    (s, ex) => s + Number(ex.postTaxTotal || 0),
                    0,
                  ),
                )}
                đ
