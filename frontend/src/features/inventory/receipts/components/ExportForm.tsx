...
export default function ExportForm() {
  ...
  useEffect(() => {
    ...
    Promise.all([getPartners(token), getProducts(token)])
      .then(([pData, prData]) => {
        setPartners(pData);
        setProducts(prData);
      })
  ...
  const vatTotal = items.reduce(
    (sum, i) => sum + i.quantity * i.price * (i.vatRate / 100),
    0,
  );
    ...
    <div className="flex flex-col gap-6 max-w-6xl mx-auto py-2">
      ...
      <div>
        ...
        <p className="text-[11px] text-slate-500 mt-1">
          Lập phiếu xuất hàng bán cho khách hàng. Thông tin tồn kho sẽ tự động
          cập nhật sau khi phê duyệt.
        </p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm border-collapse">
            <thead className="bg-slate-50/50 border-b border-slate-200 text-slate-400">
              <tr>
                <th className="px-6 py-3 font-bold text-[10px] uppercase tracking-wider">
                  Tên hàng
                </th>
                <th className="px-6 py-3 font-bold text-[10px] uppercase tracking-wider w-24">
                  ĐVT
                </th>
                <th className="px-6 py-3 font-bold text-[10px] uppercase tracking-wider w-32 text-right">
                  Số lượng
                </th>
                <th className="px-6 py-3 font-bold text-[10px] uppercase tracking-wider w-40 text-right">
                  Đơn giá
                </th>
                <th className="px-6 py-3 font-bold text-[10px] uppercase tracking-wider w-24 text-right">
                  VAT%
                </th>
                <th className="px-6 py-3 font-bold text-[10px] uppercase tracking-wider w-44 text-right">
                  Thành tiền
                </th>
            ...
            <tbody className="divide-y divide-slate-100">
              {items.length === 0 ? (
                <tr>
                  ...
                  >
                    Chưa có sản phẩm nào. Bấm &quot;Thêm sản phẩm&quot; để bắt
                    đầu.
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={item.productId} className="hover:bg-slate-50/20">
                    ...
                    <td className="px-6 py-2.5">
                      <input
                        ...
                        value={item.vatRate}
                        onChange={(e) =>
                          updateItem(idx, 'vatRate', Number(e.target.value))
                        }
                        className="w-full border border-slate-200 bg-white rounded-lg p-1.5 text-sm font-mono font-semibold text-slate-800 text-right focus:border-[#2563eb] focus:outline-none focus:ring-1 focus:ring-[#2563eb]/20 transition-colors"
                      />
                    </td>
                ...
                filteredProducts.map((p) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <div className="font-mono font-bold text-sm text-[#2563eb]">
