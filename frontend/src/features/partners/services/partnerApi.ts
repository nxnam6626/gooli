...
export async function createPartner(data: Omit<Partner, "id" | "createdAt" | "updatedAt" | "isActive">, token: string): Promise<Partner> {
...
export async function updatePartner(id: number, data: Partial<Partner>, token: string): Promise<Partner> {
...
export async function deletePartner(id: number, token: string): Promise<unknown> {
  const res = await fetch(`${API_BASE}/partners/${id}`, {
    method: "DELETE",
    headers: getHeaders(token),
  });
...
export async function createPartnerGroup(token: string, data: { code: string; name: string }): Promise<PartnerGroup> {
  const res = await fetch(`${API_BASE}/partner-groups`, {
    method: "POST",
    headers: getHeaders(token),
    body: JSON.stringify(data),
...
export async function updatePartnerGroup(token: string, id: number, data: { code?: string; name?: string }): Promise<PartnerGroup> {
...
export async function deletePartnerGroup(token: string, id: number): Promise<unknown> {
  const res = await fetch(`${API_BASE}/partner-groups/${id}`, {
    method: "DELETE",
    headers: getHeaders(token),
  });
