...
export default function ProductFilters({
  ...
  const getSortUrl = (sortValue: string) => {
    if (parsedCategoryId)
      return `/san-pham?categoryId=${parsedCategoryId}&sortBy=${sortValue}`;
  ...
  const getCategoryUrl = (catId?: number) => {
    if (catId) {
      return sortBy !== 'newest'
        ? `/san-pham?categoryId=${catId}&sortBy=${sortBy}`
        : `/san-pham?categoryId=${catId}`;
    }
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
        ...
        >
          <Link
            ...
            style={{
              color: sortBy === 'newest' ? '#E46C0A' : 'inherit',
              position: 'relative',
            }}
            className={
              sortBy === 'newest'
          ...
          <Link
            href={getSortUrl('bestseller')}
            style={{
              color: sortBy === 'bestseller' ? '#E46C0A' : 'inherit',
              position: 'relative',
            }}
            className={
              sortBy === 'bestseller'
          ...
          <Link
            href={getSortUrl('featured')}
            style={{
              color: sortBy === 'featured' ? '#E46C0A' : 'inherit',
              position: 'relative',
            }}
            className={
              sortBy === 'featured'
          ...
          <Link
            ...
            style={{
              color: sortBy === 'price_asc' ? '#E46C0A' : 'inherit',
              position: 'relative',
            }}
            className={
              sortBy === 'price_asc'
          ...
          <Link
            ...
            style={{
              color: sortBy === 'price_desc' ? '#E46C0A' : 'inherit',
              position: 'relative',
            }}
            className={
              sortBy === 'price_desc'
                ? 'flex items-center gap-1'
                : 'text-neutral-600 hover:text-neutral-900 transition-colors flex items-center gap-1'
            }
          >
            Giá cao
            <svg
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div className="relative group z-20">
          <button className="flex items-center gap-2 font-semibold text-neutral-900 py-1 hover:text-[#E46C0A] transition-colors cursor-pointer">
            {parsedCategoryId
              ? displayCategories.find((c) => c.id === parsedCategoryId)?.name
              : 'Tất cả sản phẩm'}
            <svg
              className="w-4 h-4"
              viewBox="0 0 24 24"
