...
export default function CategorySidebar({
  ...
  const [dragSubState, setDragSubState] = useState<{
    catIdx: number;
    subIdx: number;
  } | null>(null);
        ...
        <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-3 space-y-1">
          {categories.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-2 border border-dashed border-slate-200 bg-slate-50/40 p-4 rounded-xl">
              <svg
                className="w-8 h-8 text-slate-350"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
            ...
            categories.map((cat, idx) => {
              ...
              const isActive =
                editingIndex === idx && modalSel?.type === 'category';
              ...
              return (
                <div
                  ...
                  onDrop={(e) => {
                    ...
                    if (dragType === 'submenu' && dragSubState) {
                      ...
                      const { newCategories, newModalSel } = moveSubmenu(
                        categories,
                        dragSubState.catIdx,
                        dragSubState.subIdx,
                        idx,
                    ...
                    const { newCategories, newModalSel } = moveCategory(
                      categories,
                      dragCatIdx,
                      idx,
                      modalSel,
                ...
                >
                  ...
                  <IconComponent
                    size={15}
                    className={`mr-3 shrink-0 ${isFocused || isActive ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-600'} transition-colors`}
                  />

                  <span
                    className={`truncate flex-1 ${isFocused || isActive ? 'text-blue-700' : 'text-slate-650'} group-hover:text-slate-900 transition-colors`}
                  >
                  ...
                  {hasSub && (
                    <span
                      className={`px-2 py-0.5 rounded-full text-[9px] font-bold shrink-0 ml-2 group-hover:hidden transition-all ${isFocused ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500'}`}
                    >
                      {cat.subMenu?.length}
                    </span>
                  )}
          ...
          {!activeCat ? (
            <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-2 border border-dashed border-slate-200/60 bg-white/40 rounded-xl m-2 h-[calc(100%-16px)]">
              ...
              <span className="text-[11px] font-medium text-slate-550">
                Chọn danh mục chính để xem danh mục con
              </span>
            </div>
          ) : subMenu.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-2 border border-dashed border-slate-200/60 bg-white/40 rounded-xl m-2 h-[calc(100%-16px)]">
              <svg
                className="w-8 h-8 text-blue-400/50 animate-pulse"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
              ...
              <span className="text-[10px] text-slate-400">
                Bấm nút bên dưới để tạo mới danh mục con
              </span>
              <button
                type="button"
            ...
            <div className="flex flex-col gap-2">
              {subMenu.map((sub, sIdx) => {
                const isSubActive =
                  editingIndex === safeActiveIdx &&
                  modalSel?.type === 'submenu' &&
                  modalSel?.subIdx === sIdx;
                const isSubDragging =
                  dragSubState?.catIdx === safeActiveIdx &&
                  dragSubState?.subIdx === sIdx;
                const isSubDragOver =
                  dragOverSubIdx === sIdx && dragSubState?.subIdx !== sIdx;
                ...
                return (
                  ...
                  >
                    ...
                    <ArrowElbowDownRight
                      size={12}
                      className="text-blue-500/80 mr-2.5 shrink-0"
                    />
