...
export default function CategoryForm({
  ...
  const {
    ...
    if (target === "categoryImage" && targetIdx !== undefined) {
      setCategories(updateCategoryImage(categories, targetIdx, categories[targetIdx]?.image || "", posStr));
    }
  });
              ...
              {cat.image?.startsWith("data:image/") ? (
                <div className="flex-1 bg-slate-50 border border-slate-200 rounded-lg py-1.5 px-3 text-slate-600 font-semibold text-xs flex justify-between items-center select-none">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setCategories(updateCategoryImage(categories, cIdx, "", "50% 50%"));
                    }}
                ...
                <input
                  ...
                  value={cat.image || ""}
                  onChange={(e) => {
                    setCategories(updateCategoryImage(categories, cIdx, e.target.value, cat.imagePosition));
                  }}
              ...
              <label className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-lg font-bold text-xs cursor-pointer text-slate-700 flex items-center gap-1.5 select-none shrink-0 transition-colors">
                ...
                <input
                  ...
                  onChange={(e) => handleImageUpload(e, (base64) => {
                    setCategories(updateCategoryImage(categories, cIdx, base64, "50% 50%"));
                  })}
                  className="hidden"
                />
              </label>
        ...
        <div className="lg:col-span-7 space-y-5">
          <div>
            ...
            <input
              ...
              value={cat.label}
              onChange={(e) => {
                setCategories(updateCategoryLabel(categories, cIdx, e.target.value));
              }}
              className={inputCls}
            />
          </div>
          ...
          <div>
            ...
            <select
              value={cat.icon}
              onChange={(e) => {
                setCategories(updateCategoryIcon(categories, cIdx, e.target.value));
              }}
              className={inputCls}
            >
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                value={linkType}
                onChange={(e) => {
                  setCategories(updateCategoryLinkType(categories, cIdx, e.target.value));
                }}
                className={inputCls}
              >
            ...
            <div>
              ...
              {linkType === "system" && (
                <select
                  value={cat.href}
                  onChange={(e) => {
                    setCategories(updateCategoryHref(categories, cIdx, e.target.value));
                  }}
                  className={inputCls}
                >
                  {SYSTEM_PAGES.map((p) => (
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            onClick={() => {
              ...
              if (newSubIdx !== -1) {
                setModalSel({ type: "submenu", catIdx: cIdx, subIdx: newSubIdx });
              }
            }}
