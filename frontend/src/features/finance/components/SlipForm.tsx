        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            >
              ...
              {filteredPartnersForForm.map((p) => (
                <option key={p.id} value={p.id}>
                  {p.name} (Nợ hiện tại:{' '}
                  {Number(p.totalDebt).toLocaleString('vi-VN')} đ)
                </option>
              ))}
            </select>
          </div>
              ...
              {availableInvoices.length === 0 ? (
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (id) {
                      ...
                      if (inv) {
                        const debt =
                          Number(inv.postTaxTotal) - Number(inv.paidAmount);
                        setAmount(debt);
                      }
                    } else {
                      setAmount('');
                ...
                >
                  ...
                  {availableInvoices.map((inv) => {
                    const invCode =
                      'invoiceNumber' in inv ? inv.invoiceNumber : inv.code;
                    const unpaid =
                      Number(inv.postTaxTotal) - Number(inv.paidAmount);
                    return (
                      <option key={inv.id} value={inv.id}>
                        {invCode} - Giá trị:{' '}
                        {Number(inv.postTaxTotal).toLocaleString('vi-VN')} đ
            ...
            <div className="flex justify-between items-center mb-1">
              ...
              {linkType === 'DIRECT' && selectedInvoiceId && (
                <span className="text-[10px] text-slate-500 font-semibold">
                  Dư nợ hóa đơn tối đa:{' '}
                  {selectedInvoiceDebt.toLocaleString('vi-VN')} đ
                </span>
              )}
            </div>
            <input
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-gray-100">
            ...
            <button
              type="submit"
              disabled={
                submitting || (linkType === 'DIRECT' && !selectedInvoiceId)
              }
