...
export default async function ProductDetailsPage({ params }: PageProps) {
  ...
  if (dbProduct) {
    ...
    if (id === 'lam-song-ngoai-troi-hh-wood') {
      product = {
        ...
        description:
          'Vật liệu trang trí ngoại thất hiện đại, được sản xuất từ nhựa tổng hợp cao cấp hoặc gỗ nhựa composite.',
  ...
  const categoryName =
    product?.category?.name?.toUpperCase() || 'LAM GỖ NHỰA NGOÀI TRỜI';
  ...
  return (
    <main className="flex-1 bg-white min-h-screen pb-20">
      ...
      {/* Breadcrumb Bar */}
      <div
        className="w-full bg-neutral-50 border-b border-neutral-200"
        style={{ padding: '12px 0' }}
      >
        <div className="custom-container">
          <nav
            className="flex items-center text-sm text-neutral-500 uppercase tracking-wide"
            style={{ display: 'flex', alignItems: 'center', gap: '8px' }}
          >
            <Link href="/" className="hover:text-[#bc6f21] transition-colors">
              Trang chủ
            </Link>
            <span>/</span>
            <Link
              href="/san-pham"
              className="hover:text-[#bc6f21] transition-colors"
            >
              Sản phẩm
            </Link>
            <span>/</span>
