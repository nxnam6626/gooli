...
export default function ReportsPage() {
  const [activeTab, setActiveTab] = useState<'DEBT' | 'FINANCE' | 'STOCK'>(
    'DEBT',
  );
  ...
  const stockStats = React.useMemo(() => {
    ...
    const faulty = products.reduce(
      (sum, p) => sum + Number(p.faultyQty || 0),
      0,
    );
      ...
      <div className="flex justify-between items-center pb-1 border-b border-slate-200 no-print select-none">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-[11px]">
            Báo cáo tổng hợp công nợ đối tác, doanh thu tài chính dòng tiền và
            giá trị tồn kho.
          </p>
        </div>
      </div>
                    ...
                    >
                      ...
                      <div className="text-center space-y-1 pb-4 border-b border-slate-100">
                        ...
                        <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wide">
                          {startDate
                            ? `Từ ngày: ${fmtDateRange(startDate)}`
                            : ''}
                      ...
                      <div className="grid grid-cols-2 gap-4 text-[11px] text-slate-600">
                        ...
                        <div className="text-right flex flex-col justify-end">
                          ...
                          <div className="font-extrabold mt-1.5 text-slate-900 text-xs">
                            Nợ hiện tại:{' '}
                            {fmt(selectedPartnerObj?.totalDebt || 0)}đ
                          </div>
                        </div>
                      </div>
                        ...
                        <table className="w-full text-left border-collapse text-[11px]">
                          ...
                          <tbody className="divide-y divide-slate-100 text-slate-700">
                            ...
                            <tr className="bg-slate-50/35 font-bold italic">
                              <td className="py-2 px-3 border-r border-slate-100 text-slate-454 text-slate-400">
                                Đầu kỳ
                              </td>
                            ...
                            {ledgerReport.entries.length === 0 ? (
                              ...
                              (() => {
                                let currentRunning =
                                  ledgerReport.openingBalance;
                                return ledgerReport.entries.map(
                                  (entry, idx) => {
                                    currentRunning +=
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <td
                                          className="py-2 px-3 border-r border-slate-100 text-slate-600 max-w-[200px] truncate"
                    ...
                    <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs flex items-center justify-between">
                      <div className="space-y-1.5">
                        ...
                        <p className="text-slate-455 text-[10px] font-semibold">
                          Công nợ tích lũy cần thanh toán cho các nhà cung cấp
                          vật tư
                        </p>
                      </div>
                        ...
                        {topDebtors.length === 0 ? (
                          ...
                          topDebtors.map((d) => {
                            const pct = Math.round(
                              (Number(d.totalDebt) / (totalReceivables || 1)) *
                                100,
                            );
                            return (
                    ...
                    <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs space-y-4">
                      ...
                      <div className="space-y-3">
                        {topCreditors.length === 0 ? (
                          ...
                          topCreditors.map((c) => {
                            const pct = Math.round(
                              (Number(c.totalDebt) / (totalPayables || 1)) *
                                100,
                            );
                            return (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 select-none">
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-200 border-l-4 border-l-emerald-500 shadow-2xs space-y-2">
                  ...
                  <p className="text-slate-450 text-[10px]">
                    Tổng số tiền thu từ khách hàng qua phiếu thu
                  </p>
                </div>
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-200 border-l-4 border-l-rose-500 shadow-2xs space-y-2">
                  ...
                  <p className="text-slate-450 text-[10px]">
                    Tổng số tiền thanh toán cho nhà cung cấp qua phiếu chi
                  </p>
                </div>
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-200 border-l-4 border-l-slate-900 shadow-2xs space-y-2">
                  ...
                  <div
                    className={`text-2xl font-black font-mono ${financialReport.netFlow >= 0 ? 'text-slate-900' : 'text-rose-700'}`}
                  >
                    {financialReport.netFlow >= 0 ? '+' : ''}{' '}
                    {fmt(financialReport.netFlow)}đ
                  </div>
                  <p className="text-slate-450 text-[10px]">
                    Hiệu số Thu nhập trừ Chi phí thực tế trong kỳ báo cáo
                  </p>
                </div>
              </div>
                    ...
                    <div className="h-64 flex items-end justify-around border-b border-slate-200 pb-2 pt-6">
                      {monthlyFlows.map((flow, idx) => {
                        ...
                        return (
                          <div
                            key={idx}
                            className="flex flex-col items-center gap-1 w-1/6 group"
                          >
                            {/* Combined bars */}
                            ...
                            <span className="text-[10px] font-bold text-slate-500 mt-2 select-none">
                              {flow.month}
                            </span>
                          </div>
                        );
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-200 flex justify-between items-center">
                    <div>
                      ...
                      <div className="text-[10px] text-slate-400 font-semibold mt-0.5">
                        Giá trị giao dịch qua tài khoản ngân hàng
                      </div>
                    </div>
                    <div
                      className={`text-sm font-extrabold font-mono ${financialReport.bankTransferTotal >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                    >
                      {financialReport.bankTransferTotal >= 0 ? '+' : ''}
                      {fmt(financialReport.bankTransferTotal)}đ
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-200 flex justify-between items-center">
                    <div>
                      ...
                      <div className="text-[10px] text-slate-400 font-semibold mt-0.5">
                        Giá trị giao dịch thanh toán bằng tiền mặt tại két
                      </div>
                    </div>
                    <div
                      className={`text-sm font-extrabold font-mono ${financialReport.cashTotal >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                    >
                      {financialReport.cashTotal >= 0 ? '+' : ''}
                      {fmt(financialReport.cashTotal)}đ
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 select-none">
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs flex items-center justify-between">
                  <div className="space-y-1.5">
                    ...
                    <span className="text-2xl font-black text-slate-900 block font-mono">
                      {fmt(totalStockValue)}đ
                    </span>
                    <p className="text-slate-450 text-[10px] font-semibold">
                      Ước tính theo đơn giá định mức của sản phẩm
                    </p>
                  </div>
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs flex items-center justify-between">
                  <div className="space-y-1.5">
                    ...
                    <span className="text-2xl font-black text-slate-900 block font-mono">
                      {stockStats.total.toLocaleString()}
                    </span>
                    <div className="text-[10px] font-bold text-slate-500 flex gap-2">
                      <span className="text-emerald-600">
                        Đạt chuẩn: {stockStats.standard.toLocaleString()}
                      </span>
                      <span className="text-rose-600">
                        Lỗi/Hỏng: {stockStats.faulty.toLocaleString()}
                      </span>
                    </div>
                  </div>
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs flex items-center justify-between">
                  <div className="space-y-1.5">
                    <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">
                      TỶ LỆ LỖI/HỎNG (FAULTY RATE)
                    </span>
                    <span className="text-2xl font-black text-rose-600 block font-mono">
                      {stockStats.rate}%
                    </span>
                    <p className="text-slate-450 text-[10px] font-semibold">
                      Tỷ lệ hao hụt lỗi hỏng trên tổng số hàng hiện tại
                    </p>
                  </div>
                  <div className="w-12 h-12 rounded-xl bg-rose-50 text-rose-600 flex items-center justify-center shrink-0">
                    ...
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 font-bold uppercase tracking-wider text-[10px] select-none">
                        ...
                        <th className="py-3 px-4 text-right">
                          Tồn kho chuẩn (Đạt chuẩn)
                        </th>
                        <th className="py-3 px-4 text-right">
                          Tồn hàng hỏng (Lỗi/Hỏng)
                        </th>
                      ...
                      {products.map((prod) => (
                        ...
                        >
                          ...
                          <td className="py-2.5 px-4 text-right font-bold font-mono text-rose-600">
                            {Number(prod.faultyQty || 0).toLocaleString(
                              'vi-VN',
                            )}
                          </td>
