...
export default function PartnerGroupsPage() {
  ...
  // Calculate average discount from discount policies
  ...
  // Get icon style classes for folder based on group name
                  ...
                  {filteredItems.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="py-3.5 px-5">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${getGroupIconBg(item.name)}`}>
                            {getGroupIcon(item.name)}
                          </div>
                          <div>
                            <button
                              onClick={() => handleOpenEdit(item)}
                              className="font-bold text-slate-900 hover:text-[#2563eb] hover:underline"
                            >
                              {item.name}
                            </button>
                          </div>
                      ...
                      <td className="py-3.5 px-5 text-slate-500 font-medium max-w-xs truncate" title={item.description || ''}>
                        {item.description || '-'}
                      </td>
                      <td className="py-3.5 px-5">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="space-y-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="grp_code" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Mã nhóm *</label>
                  <input
                    id="grp_code"
                    type="text"
                    required
                ...
                {/* Name */}
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="grp_name" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tên nhóm *</label>
                  <input
                    id="grp_name"
                    type="text"
                    required
                ...
                {/* Description */}
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="grp_desc" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Mô tả</label>
                  <input
                    id="grp_desc"
                    type="text"
                ...
                {/* Policies */}
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="grp_policy" className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Chính sách / Ưu đãi</label>
                  <input
                    id="grp_policy"
                    type="text"
